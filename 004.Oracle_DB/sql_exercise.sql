...
select employee_id 사원번호, last_name||first_name 사원이름, salary 급여
from employees;
...
select last_name||first_name 이름, department_id 부서번호, salary 월급
from employees
...
SELECT last_name||first_name 이름, employee_id 사원번호, DEPARTMENT_ID 부서번호
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, HIRE_DATE 사원번호, DEPARTMENT_ID 부서번호
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, SALARY 급여, COMMISSION_PCT 상여, (SALARY+COMMISSION_PCT) 총액
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, SALARY 급여, (SALARY*0.13) 보너스금액, DEPARTMENT_ID 부서번호
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, DEPARTMENT_ID 부서번호, SALARY 급여, (SALARY*12+(SALARY*1.5)) 연봉
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, SALARY 급여, (SALARY/12/5) 시간당임금
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, SALARY 급여, ROUND(SALARY*0.05,2) 회비
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, HIRE_DATE 입사일, SYSDATE 현재일,
(SYSDATE-HIRE_DATE) 근무일수,
TO_CHAR(SYSDATE,'YYYY')-TO_CHAR(HIRE_DATE,'YYYY') 근무년수,
FLOOR((SYSDATE-HIRE_DATE)/30) 근무월수 
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, HIRE_DATE 입사일, ROUND(SYSDATE-HIRE_DATE,0) 근무일수
FROM EMPLOYEES
...
SELECT FIRST_NAME||LAST_NAME 이름, TO_CHAR(HIRE_DATE,'yyyy"년"/fmmm"월"/dd"일"') 입사일
FROM EMPLOYEES
...
SELECT TRUNC(AVG(SALARY)) 평균, MAX(SALARY) 최고, MIN(SALARY) 최저, COUNT(*) 인원수 
FROM EMPLOYEES
WHERE DEPARTMENT_ID =10;
...
SELECT DEPARTMENT_ID 부서번호, JOB_ID 업무명, COUNT(*) 
FROM EMPLOYEES
...
SELECT DEPARTMENT_ID 부서번호, SALARY 전체월급, MAX(SALARY) 최고월급, MIN(SALARY) 최저월급, ROUND(AVG(SALARY),1) 평균월급 
FROM EMPLOYEES
