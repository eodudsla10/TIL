...
class StudentService :
    ...
    students=[] #클래스 변수    :프로그램 시작시 파일에서 student객체를 읽어서 한번 초기화

    # 수강생 등록 : list students에 id 중복체크하고 존재하지 않는 경우 정보 저장하고 message 리턴, 존재하는 경우 DuplicateException 발생
    def register(self, student) :
        ...
        if (index < 0) :
            ...
            try:
                raise DuplicateException(student.id)    #raise -> 사용자가 직접 에러를 발생
            except DuplicateException as inputerror: #str화 된 inputerror을 넘겨준다
                return str(inputerror)
    # 수강생 목록 : list students 목록 리턴
    def getAllStudents(self) :
        return StudentService.students

    # 수강생 수정 : id 검색해서 전공정보 수정하고 message 리턴, 존재하지 않는 경우 IDNotFoundException 발생
